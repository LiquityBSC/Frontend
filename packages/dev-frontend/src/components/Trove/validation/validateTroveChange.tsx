...
const TroveChangeDescription: React.FC<TroveAdjustmentDescriptionParams> = ({ params }) => (
  <ActionDescription>
    {params.depositCollateral && params.borrowLUSD ? (
      <>
        You will deposit <Amount>{params.depositCollateral.prettify()} BNB</Amount> and receive{" "}
        <Amount>
      ...
      <>
        You will deposit <Amount>{params.depositCollateral.prettify()} BNB</Amount> and pay{" "}
        <Amount>
          {params.repayLUSD.prettify()} {COIN}
        </Amount>
      </>
      ...
      <>
        You will receive <Amount>{params.withdrawCollateral.prettify()} BNB</Amount> and{" "}
        <Amount>
          {params.borrowLUSD.prettify()} {COIN}
        </Amount>
      </>
      ...
      <>
        You will deposit <Amount>{params.depositCollateral.prettify()} BNB</Amount>
      </>
    ) : params.withdrawCollateral ? (
      <>
        You will receive <Amount>{params.withdrawCollateral.prettify()} BNB</Amount>
      </>
    ) : params.borrowLUSD ? (
      <>
        You will receive{" "}
  ...
  if (depositCollateral.gt(accountBalance)) {
    return (
      <ErrorDescription>
        ...
        <Amount>{depositCollateral.sub(accountBalance).prettify()} BNB</Amount>.
      </ErrorDescription>
    );
  }
