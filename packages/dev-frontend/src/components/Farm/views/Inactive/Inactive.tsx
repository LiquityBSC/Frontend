...
const uniLink = (lusdAddress: string) => `https://exchange.pancakeswap.finance/#/add/BNB/${lusdAddress}`;
...
export const Inactive: React.FC = () => {
  ...
  return (
    <Card>
      <Heading>
        Pancake Liquidity Farm
        <Flex sx={{ justifyContent: "flex-end" }}>
          <RemainingLQTY />
        </Flex>
      </Heading>
      <Box sx={{ p: [2, 3] }}>
        <InfoMessage title="You aren't farming LBSC.">
          <Paragraph>You can farm LBSC by staking your Pancake BNB/FUSD LP tokens.</Paragraph>

          <Paragraph sx={{ mt: 2 }}>
            You can obtain LP tokens by adding liquidity to the{" "}
            <Link href={uniLink(addresses["lusdToken"])} target="_blank">
            BNB/FUSD pool on Pancake. <Icon name="external-link-alt" size="xs" />
            </Link>
          </Paragraph>
        </InfoMessage>
